      ...
      return Promise.all([
        ...
        .then(([data, res]) => {
          ...
          res.body.forEach(function (item, i) {
            ...
            expect(mongo.ObjectId(item.folderId)).to.eql(data[i].folderId);
    ...
    it('should return correct search results for a searchTerm query', function () {
      ...
      return Promise.all([dbPromise, apiPromise])
        .then(([data, res]) => {
          ...
          res.body.forEach(function (item, i) {
            expect(item).to.be.a('object');
            expect(item).to.include.all.keys('id', 'title', 'userId', 'createdAt', 'updatedAt', 'tags'); // Note: folderId and content are optional
            expect(item.id).to.equal(data[i].id);
            expect(item.title).to.equal(data[i].title);
            expect(item.content).to.equal(data[i].content);
            expect(mongo.ObjectId(item.userId)).to.eql(data[i].userId);
            expect(new Date(item.createdAt)).to.eql(data[i].createdAt);
            expect(new Date(item.updatedAt)).to.eql(data[i].updatedAt);
          });
        });
    ...
    it('should return correct notes', function () {
      ...
      return Note.findOne({ userId: user.id })
        ...
        .then((res) => {
          ...
          expect(mongo.ObjectId(res.body.userId)).to.eql(data.userId);
